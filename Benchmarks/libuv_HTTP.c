// Based on: https://medium.com/@padam.singh/an-event-driven-tcp-server-using-libuv-50cce9a473c0
...
static void
on_close_free (uv_handle_t * handle)
{
  free (handle);
}
 ...
 */
static void
close_data (struct client_request_data *data)
{
  if (!data)
    return;
  if (data->client)
...
static void
after_process_command (uv_work_t * req, int status)
{
  struct client_request_data *data;
  data = req->data;
 ...
 */
static void
process_command (uv_work_t * req)
{
  struct client_request_data *data;
  char *x;
  data = req->data;
  // Do the actual time-consuming request processing here
  data->response = (char *)http_reply;
}
  ...
  if (nread == -1 || nread == UV_EOF)
      ...
      if (!data->work_started && data->text_len)
	{
	  ...
	  uv_queue_work (loop, data->work_req,
			 process_command, after_process_command);
	}
    }
}
...
static void
client_timeout_cb (uv_timer_t * handle)
{
  struct client_request_data *data;
