...
namespace IODash {
	enum class AddressFamily : uint16_t {
		Unspecified = AF_UNSPEC, Unix = AF_UNIX, IPv4 = AF_INET, IPv6 = AF_INET6
	};
	...
	public:
		SocketAddress(void *__sa_ptr) : sa_ptr(reinterpret_cast<sockaddr *>(__sa_ptr)) {
		...
		AddressFamily family() const noexcept {
			return (AddressFamily) sa_ptr->sa_family;
		}
		...
		friend void swap(SocketAddress &first, SocketAddress &second) noexcept {
			using std::swap;

			swap(first.sa_ptr, second.sa_ptr);
		}
	};

	template<>
	class SocketAddress<AddressFamily::IPv4> : public SocketAddress<AddressFamily::Unspecified> {
	private:
		sockaddr_in sa;

	public:
		SocketAddress() : SocketAddress<AddressFamily::Unspecified>::SocketAddress(&sa) {
			reset();
		}

		SocketAddress(sockaddr_in *__sa) : SocketAddress() {
			reset();
			sa.sin_port = __sa->sin_port;
		...
		auto port() noexcept {
			struct {
				...
				operator uint16_t() {
					return be16toh(p->sa.sin_port);
				}

				uint16_t operator=(uint16_t s) {
					p->sa.sin_port = htobe16(s);
					return s;
				}
			} ret{this};
		...
		void from_string(const std::string &__addr_str) {
			...
			if (p != std::string::npos) {
				...
				inet_pton(AF_INET, buf.c_str(), &sa.sin_addr);
			} else {
				inet_pton(AF_INET, __addr_str.c_str(), &sa.sin_addr);
			}


		}

		SocketAddress(const SocketAddress &other) noexcept : SocketAddress() {
			sa = other.sa;
		}
		...
		friend inline bool operator<(const SocketAddress &lhs, const SocketAddress &rhs) {
			return std::tie(lhs.sa.sin_addr.s_addr, lhs.sa.sin_port) <
			       std::tie(rhs.sa.sin_addr.s_addr, rhs.sa.sin_port);
		}
	...
	template<>
	class SocketAddress<AddressFamily::IPv6> : public SocketAddress<AddressFamily::Unspecified> {
	private:
		sockaddr_in6 sa;
	...
	public:
		...
		SocketAddress(sockaddr_in6 *__sa) : SocketAddress() {
			reset();
			sa.sin6_port = __sa->sin6_port;
			sa.sin6_flowinfo = __sa->sin6_flowinfo;
			sa.sin6_scope_id = __sa->sin6_scope_id;
			memcpy(&sa.sin6_addr, &__sa->sin6_addr, sizeof(in6_addr));
		}
		...
		auto port() noexcept {
			struct {
				...
				operator uint16_t() {
					return be16toh(p->sa.sin6_port);
				}

				uint16_t operator=(uint16_t s) {
					p->sa.sin6_port = htobe16(s);
					return s;
				}
			} ret{this};
		...
		std::string to_string(bool __with_port = true) const {
			...
			if (__with_port) {
				buf[0] = '[';
				inet_ntop(AF_INET6, &sa.sin6_addr, buf + 1, INET6_ADDRSTRLEN);
		...
		void from_string(const std::string &__addr_str) {
			...
			if (p != std::string::npos) {
				...
				inet_pton(AF_INET6, buf.data() + 1, &sa.sin6_addr);
			} else {
				inet_pton(AF_INET6, __addr_str.c_str(), &sa.sin6_addr);
			}
		...
		friend inline bool operator<(const SocketAddress &lhs, const SocketAddress &rhs) {
			auto *a1 = (uint8_t *) &lhs.sa.sin6_addr;
			auto *a2 = (uint8_t *) &rhs.sa.sin6_addr;
	...
	template<>
	class SocketAddress<AddressFamily::Unix> : public SocketAddress<AddressFamily::Unspecified> {
	private:
		sockaddr_un sa;
	...
	public:
		...
		SocketAddress(sockaddr_un *__sa) : SocketAddress() {
			reset();
			strcpy(sa.sun_path, __sa->sun_path);
		}
		...
		void from_string(const std::string &__addr_str) {
			strncpy(sa.sun_path, __addr_str.c_str(), sizeof(sa.sun_path) - 1);
		}
		...
		friend inline bool operator<(const SocketAddress &lhs, const SocketAddress &rhs) {
			std::string_view a(lhs.sa.sun_path), b(rhs.sa.sun_path);
